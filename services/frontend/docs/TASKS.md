...
- [ ] Баги длинного названия трека:
    - Карточка в основном списке треков - название трека увеличивает размер карточки. Должно либо прятаться за ..., либо переноситься на следующую строчку
    - В списке результатов распознавания - ужимает до минимума иконку галочки, либо крестика
- [ ] При распознавании треков картинки подгружаются не в процессе сопоставления, а после. Как результат - мы последовательно, без DDOS'а ищем каждый трек отдельно, а потом запрашиваем разом кучей запросов картинки
- [ ] При экспорте плейлиста из внешних сервисов нет никаких лоадеров или других индикаторов, что процесс идет, а не произошла ошибка
- [ ] При карточке детальной информации при распознавании трека в Spotify не хватает информации об исходном треке. В результате сложно выбрать подходящий трек
- [ ] Написать руководство пользователя на главном экране + добавить возможность попадать на него. А еще лучше Wizard, который бы провел пользователя по созданию первого плейлиста
- [ ] При сопоставлении бывают треки, у которых ну точно должен был найден результат, но в итоге - нет. Нужно понять, может быть можно как-то уточнить поиск без ложных привязок
...
- [ ] Добавить счетчик сколько треков всего не просто в плейлисте, но и в каждом подключенном сервисе
- [ ] Подключить нейроревью
