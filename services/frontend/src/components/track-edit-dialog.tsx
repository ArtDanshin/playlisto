...
import type { Track, SpotifyTrackData } from "@/lib/m3u-parser"
...
export function TrackEditDialog({ track, onTrackUpdate, children }: TrackEditDialogProps) {
  ...
  const handleSearchInSpotify = async () => {
    ...
    try {
      ...
      const response = await SpotifyService.searchTracks(query, 10)
